...
from rrl_td3.policies import Actor, Critic, MlpPolicy, CnnPolicy, MultiInputPolicy, ResidualTD3Policy

SelfResidualTD3 = TypeVar("SelfResidualTD3", bound="ResidualTD3")

class ResidualTD3(OffPolicyAlgorithm):
    """
    ...
    def __init__(
            self,
            policy: Union[str, type[ResidualTD3Policy]],
    ...
    def predict(
        ...
        """
        return self.policy.predict(observation, state, episode_start, deterministic, total_action)
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            traction_action = self.policy.traction.predict(self._last_obs)[np.newaxis]
        else:
            # Note: when using continuous actions,
            # we assume that the policy uses tanh to scale the action
            # We use non-deterministic action in the case of SAC, for TD3, it does not matter
            assert self._last_obs is not None, "self._last_obs was not set"
            unscaled_action, _ = self.predict(self._last_obs, deterministic=False, total_action=False)
            traction_action = self.policy.traction.predict(self._last_obs)[np.newaxis]
        ...
        if isinstance(self.action_space, spaces.Box):
            ...
            action = (1-self.policy.alpha) * traction_action + self.policy.alpha * action
        else:
