...
class IBRLPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        test = [param for name, param in self.bc_policy.features_extractor.named_parameters()
                if "features_extractor" not in name]
        self.actor = self.make_actor(features_extractor=self.bc_policy.features_extractor)
        test2 = [param for name, param in self.actor.features_extractor.named_parameters()
                if "features_extractor" not in name]
        ...
        # HBY Actor target
        # TODO: copy feature extractor from bc policy
    ...
    def _predict(self, observation: PyTorchObs, deterministic: bool = False) -> th.Tensor:
        # TODO: ibrl with actor and ibrl with actor target
        ...
        return greedy_action  # self.actor(observation, deterministic)
